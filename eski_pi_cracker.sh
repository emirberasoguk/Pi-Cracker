...
check_dependencies() {
  ...
  if command -v nvidia-smi &>/dev/null || lspci | grep -i "vga" | grep -i "nvidia" &>/dev/null; then
    has_gpu=true
  fi
}
...
run_attack() {
  ...
  for wordlist in "$WORDLIST_DIR"/*.txt; do
    if [ -f "$wordlist" ]; then
      ...
      if [ "$mode" == "hashcat" ]; then
        ...
        if [ $? -eq 0 ]; then
          echo -e "\n${GREEN}[!!!] ŞİFRE BULUNDU (Hashcat)${NC}"
          hashcat -m 22000 -a 0 "$target_file" --show
          exit 0
        fi
      else
        aircrack-ng -w "$wordlist" "$target_file" -l "bulunan_sifre.txt"
        if [ $? -eq 0 ] && [ -f "bulunan_sifre.txt" ]; then
          echo -e "\n${GREEN}[!!!] ŞİFRE BULUNDU (Aircrack)${NC}"
          cat "bulunan_sifre.txt"
          exit 0
        fi
      fi
    fi
  ...
  while true; do
    ...
    if [ "$mode" == "hashcat" ]; then
      ...
      hc_ret=$?
      if [ $hc_ret -eq 0 ]; then
        echo -e "\n${GREEN}[!!!] ŞİFRE BULUNDU${NC}"
        hashcat -m 22000 -a 0 "$target_file" --show
        exit 0
      elif [ $hc_ret -ne 1 ]; then
        echo -e "${RED}[HATA] Hashcat hatası (Kod: $hc_ret)${NC}"
        exit 1
      fi
    else
