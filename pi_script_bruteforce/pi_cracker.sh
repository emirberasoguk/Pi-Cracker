...
MODE="aircrack"
if [ "$UI_ATTACK_MODE" == "1" ]; then
  MODE="hashcat"
elif [ "$UI_ATTACK_MODE" == "2" ]; then
  MODE="aircrack"
elif [ "$UI_ATTACK_MODE" == "0" ]; then
  # AUTO: Prefer Hashcat if file is hc22000 or if GPU exists
  if [[ "$TARGET_FILE" == *".hc22000" ]]; then
    MODE="hashcat"
  elif command -v hashcat &>/dev/null && command -v nvidia-smi &>/dev/null; then
    MODE="hashcat"
...
while true; do
  ...
  if [ "$MODE" == "hashcat" ]; then
    ...
    cmd="$ENGINE_BIN $PASS_LEN $NUM_PASS /dev/stdout $GEN_ARGS | awk '!seen[\$0]++' | hashcat -m 22000 -a 0 \"$TARGET_FILE\" --session pi_hybrid --status --status-timer 1 2>&1 | tee -a \"$LOG_FILE\"; echo \${PIPESTATUS[2]} > \"$TMP_DIR/last_exit_code\""
    eval "$cmd"
    ...
    cmd="$ENGINE_BIN $PASS_LEN $NUM_PASS /dev/stdout $GEN_ARGS | awk '!seen[\$0]++' | aircrack-ng -w - $AIRCRACK_TARGET \"$TARGET_FILE\" -l \"$CRACKED_KEY\" 2>&1 | tee -a \"$LOG_FILE\""
    eval "$cmd"
    if [ -f "$CRACKED_KEY" ]; then
      handle_success
    fi
